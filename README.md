...
The Online Food Monitoring System solves these problems by providing a simple digital solution to manage the core operations of a restaurant. It allows staff to add and track food items in inventory, place and view customer orders, generate bills, and collect feedback — all stored securely in files so data is never lost between sessions.

In real life, systems like this are the foundation of platforms like Zomato and Swiggy. When you place an order online, the restaurant's system checks inventory, confirms the order, generates a bill, and logs your details — exactly what this program simulates on a smaller scale.
...
## Proposed Modules
The system is divided into four main modules, each handling a specific aspect of restaurant management:
...
This module generates a bill for a placed order by entering the Order ID.
It displays the item, quantity, and total price in a formatted receipt, and saves the bill to bill.txt for future reference. Duplicate billing is prevented automatically.

### 4. Customer Feedback Module
...
All three modules save their data to text files — inventory.txt, order.txt, bill.txt, and feedback.txt so that no data is lost when the program is closed. Data is automatically loaded back when the program starts again.
